...
```{r weekly_untriaged_failure_rate}
df <- runs_data %>%
  filter(Result == "Fail") %>%
  mutate(
    `Failure Attribution` = if_else(
  ...
  summarise(
    ...
    Needs_Triage_Rate = if_else(Failures == 0, 0, Needs_Triage / Failures),
    .groups = "drop"
  ) %>%
  arrange(Week)
...
p <- ggplot(
  ...
  aes(
    ...
    text = paste0(
      ...
      "<br>Needs Triage: ", scales::percent(Needs_Triage_Rate, accuracy = 0.1),
      "<br>Failures: ", Failures,
      "<br>Untriaged Failures: ", Needs_Triage
    )
  )
) +
  geom_col(fill = "#7F7F7F", width = 5) +
  geom_hline(yintercept = 0.10, linetype = "dashed", color = "#f0ad4e") +
  scale_y_continuous(labels = percent, limits = c(0, 1)) +
  labs(x = NULL, y = "% of Failures") +
  BASE_THEME +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplotly(p, tooltip = "text") %>%
  layout(
    title = plotly_title("Weekly Share of Failures Still Needing Triage"),
    plot_bgcolor = "#1f1f1f",
    paper_bgcolor = "#1f1f1f",
    yaxis = list(tickformat = ".0%")
  ) %>%
